...
public:
	...
	Material(const Vector3f d, const Vector3f ss, const float a)
		: Kd(d), Ks(ss), alpha(a), texid(0), Kt(ZEROES), indexOfRefraction(1), Pd(Kd.norm()), Pr(Ks.norm()), Pt(0), s(Pd + Pr) {
		Pd /= s;   Pr /= s;
	}
	...
	Material(Material& o) { Kd = o.Kd;  Ks = o.Ks;  alpha = o.alpha;  texid = o.texid; indexOfRefraction = o.indexOfRefraction; Kt = o.Kt; Pd = o.Pd; Pr = o.Pr; Pt = o.Pt; s = o.s; }
...
public:
	...
	virtual bool Intersect(const Ray& r, IntersectRecord* i) { return false; }
	virtual Box3d bbox() const = 0;
	virtual float Estimate(const Vector3f& p) { return -1; }
	Material* mat;
	Vector3f center;
...
public:
	...
	const IntersectRecord& operator=(IntersectRecord& other) {
		normal = other.normal; t = other.t;
...
public:
	...
	float Estimate(const Vector3f& P)
	{
		...
		//Vector3f P1 = r.pointAtDistance(i->t);
		//P = i->intersectionPoint;
...
public:
	...
	float Estimate(const Ray& r, IntersectRecord* i)
	{
		return INF;
	}
...
class RayMarching : public Shape
{
	...
	//virtual Box3d bbox() const = 0;
	////	virtual float Estimate(const Ray& r, IntersectRecord* i);
	//Material* mat;
	//Vector3f center;
	////	virtual Shape& operator=(Shape& other) { *mat = *(other.mat); }
...
public:
	...
	bool Intersect(const Ray& r, IntersectRecord* i)
	{
		...
		t = ϵ // ϵ=10−3 is the small step distance to get past the starting point
		while true:
		{
		P = O + t D
		dt = B .Distance ( P)
		t = t + |dt| // The meat of the algorithm!
		...
		if P > DistanceLimit: return NO INTERSECTION // Suggest 10e4
		if t=ϵ : return NO INTERSECTION // Avoid intersecting the originating object

		// Calculate Normal (via central difference)
		P = O+t D
		h = 10−3
		nx=Distance (Px+h, Py, Pz) − Distance( Px−h, P y, Pz)
		ny=Distance (Px, Py+h , Pz) − Distance( Px, Py−h , Pz)
		nz=Distance( Px, Py,Pz+h) − Distance (Px,P y, Pz−h)
		...
		*/
		...
		while (true)
		{
			...
			else if (count > TOO_MANY_STEPS)   // ||  P > DistanceLimit ???
			{
				i = NULL;
				return false;
			}
...
public:
	...
	/*
	Depth-of-field (An easy feature for project 5.)
		Specify the distance to the in-focus plane: f (in real world coordinate units)
		Relative width of disk around eye to spread ray starts: w (0.3=lots, 0.1=little)
		Do the pixel calculation in the focal plane (instead of the one-unit-out plane)
			P = E + f d x X + f d y Y + f Z
		Adjust the starting point in a disk around the eye similarly:
			r = w √(ξ1) (Sqrt causes uniform distribution across the disk.)
			θ = 2π ξ2
			(r x , r y) = (r cosθ , r sin θ)
			E = E + r x X + r yY
		Use ray	starting at: E + r x X + r yY
		in direction ̂P− ̂E = ( f d x−r x ) X + ( f dy−r y )Y + f Z
